...
**Fix front ⏳ pendiente:** el modal de abono ya captura `err?.error?.mensaje` con Swal — pero hay que verificar que el mensaje llegue correctamente dado el fix global de `TokenInterceptor` (ver L-G1 en CLAUDE.md). Si el back devuelve `Content-Type: application/json`, el mensaje debería mostrarse automáticamente.
