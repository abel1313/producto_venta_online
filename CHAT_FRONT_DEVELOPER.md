...
### Comportamiento del email de notificación (para que el front no lo rompa)

El backend manda email al admin cuando llega un visitante **solo si el admin no está conectado al WebSocket del chat**. El front marca al admin como conectado enviando este mensaje WebSocket al conectarse al panel:

```
SEND /app/chat.admin.conectado
```

Si el front deja de mandar ese mensaje, el admin recibirá emails aunque esté en el panel.
Si el front lo manda correctamente, el backend suspende los emails mientras el admin esté conectado.
