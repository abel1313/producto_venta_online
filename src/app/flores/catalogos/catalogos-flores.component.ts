...
import {
  IAccesorioRamo, ICantidadFlor, IFraseListon, ITipoFlor
} from '../models/flores.model';
import { FloresService } from '../service/flores.service';

type Tab = 'tipos' | 'cantidades' | 'accesorios' | 'frases';
...
export class CatalogosFloresComponent implements OnInit {
  ...
  nuevoAccesorio  = { nombre: '', precio: 0, admiteTextoLibre: false, esPapel: false };
  ...
  agregarAccesorio(): void {
    ...
    this.ejecutar(
      ...
      () => { this.nuevoAccesorio = { nombre: '', precio: 0, admiteTextoLibre: false, esPapel: false }; },
  ...
  editar(item: ITipoFlor | ICantidadFlor | IAccesorioRamo | IFraseListon): void {
  ...
  cancelarEdicion(): void {
    ...
    this.editTipo = this.editCantidad = this.editAccesorio = this.editFrase = null;
  }
  ...
  toggleActivo(item: ITipoFlor | ICantidadFlor | IAccesorioRamo | IFraseListon): void {
    if (this.guardando) return;
